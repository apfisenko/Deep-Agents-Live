...
### Состав работ
- [ ] Изучить входы: [vision](../../../concept/vision.md) (сценарии С-*/П-*), реальные диалоги (`dataset/analysis/chats/`, `dataset/source/`), отчёт [`dataset/analysis-report.md`](../../../../dataset/analysis-report.md), KB [`data/`](../../../../data/)
...
### Состав работ
- [x] `run_experiment.py`: task → Agent Core с `config_id` (E-3/E-6); имя рана и `run_metadata` по E-9
- [x] Item- и run-evaluators по metrics-map, включая `task_error`/`error_rate` (E-19), по [шаблону evaluators](../../../../.methodology/templates/eval/evaluators-template.py)
- [x] Run-level ключи — только в `run_metadata` (E-25); версия `langfuse` запинена в lockfile
- [x] Прогон baseline; evidence (Dataset Run + drill-down) в summary
...
### Артефакты
`evals/scripts/run_experiment.py`, `evals/scripts/evaluators.py`, lockfile, evidence в [summary](tasks/05-runner-baseline/summary.md).
...
### Состав работ
- [x] `analyze_run.py`: сводка метрик, распределение, топ-5 худших items со слоем провала по трейсу
- [x] Отчёт в `evals/reports/`; drill-down через LangGraph trace + tool spans
- [x] Самопроверка по DoD

### DoD
**Агент:** отчёт содержит все разделы; слой провала у каждого из топ-5 обоснован ссылкой на trace/spans.
**Пользователь:** прочитать отчёт; согласовать top-исправления (вход в eval-fix loop, v0.2).

### Артефакты
[analyze_run.py](../../../../evals/scripts/analyze_run.py), [analysis-...191628Z.md](../../../../evals/reports/analysis-baseline-react-inmemory--e2e-qa--5eedd7a1--20260615T191628Z.md), [summary](tasks/06-analyze/summary.md).
...
**Sprint eval-01 закрыт 2026-06-15.**

Реализован vertical slice: `validate → sync → experiment → analyze` на `e2e/e2e-qa/v001` (24 items). Baseline run `...191628Z`: correctness **0.322**, faithfulness **0.699**, error_rate **0**. Главный драйвер провалов — **kb_gap** (11/24).

**В v0.2:** eval-fix loop (KB + prompt + compare_runs), остальные датасеты по dataset-map.

Отчёт: [analysis-...191628Z.md](../../../../evals/reports/analysis-baseline-react-inmemory--e2e-qa--5eedd7a1--20260615T191628Z.md)
