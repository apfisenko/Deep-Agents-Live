"""Юнит-тесты drill-модуля гоняются из venv стенда: `cd devstand && uv run pytest ../tests`."""
...
# tests/ -> drill/ -> companion/ -> course-companion/ (корень с пакетом companion)
sys.path.insert(0, str(Path(__file__).resolve().parents[3]))

from companion.drill import AxisOption, CaseAxis, DrillCase  # noqa: E402
from companion.drill.case import RATIONALE_KEY, SUBMIT_EVENT  # noqa: E402
from companion.drill.generator import CATALOG_ID, SPEC_VERSION  # noqa: E402
...
def valid_form_payload(case: DrillCase, *, with_button: bool = True) -> str:
    """«Идеальный ответ LLM»: текст + <a2ui-json>-блок с формой кейса."""
    axis = case.axes[0]
...
def make_token_factory(*payloads: str, chunk_size: int = 17):
    """Фабрика токен-стримов: i-й вызов отдаёт i-й payload чанками.

    Последний payload повторяется; число вызовов — в `factory.state["calls"]`.
    """
    state = {"calls": 0}
