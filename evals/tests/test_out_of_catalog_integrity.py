"""Integrity tests for edge/out-of-catalog v001."""
...
def test_out_of_catalog_structure() -> None:
    ...
    for item in ds.items:
        ...
        assert item.metadata.intent == "out-of-catalog"
        assert item.metadata.nearest_product_id
        assert item.metadata.facts
        assert item.expected_output.answer.strip()


def test_out_of_catalog_nearest_sku_coverage() -> None:
    ds = load_manifest(MANIFEST)
    nearest = {item.metadata.nearest_product_id for item in ds.items}
    assert len(nearest) >= 3


def test_out_of_catalog_integrity_with_review() -> None:
    ds = load_manifest(MANIFEST)
...
def test_out_of_catalog_reviewed_by_gate() -> None:
    ds = load_manifest(MANIFEST)
