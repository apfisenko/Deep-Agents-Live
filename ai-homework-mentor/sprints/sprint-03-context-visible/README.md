...
| Артефакт «боль S3» для сравнения | Оптимизация стоимости моделями (S9) |
...
|---|----------|-----------------|
| 1 | Прогон S2-потока на согласованном **большом** репо завершается (или корректно деградирует с логом) одним агентом | `.\make.ps1 run -- ... -Verbose` на big fixture/URL |
| 2 | Суммаризация и/или компактизация подключены по порогам из YAML | События CE в логе при искусственно низком пороге в тесте |
| 3 | Offload в файлы виден (что вынесено, путь) | Verbose: список offload-событий + файлы в workspace |
| 4 | Verbose показывает размер контекста / токены **по шагам** | Таблица/лента в CLI; снимок в `workspace/.../notes/context_trace.json` (или аналог) |
| 5 | Зафиксирована «боль S3» для контраста с S4 | `docs/pain-s3.md` (+ опц. числа: max tokens, число summarize) |
| 6 | Субагентов по-прежнему нет | Verbose без handoff subagents |
...
После «ок» по S3 → разворот **S4** (`sprint-04-subagents`): изоляция reviewer-субагентов и контраст с `pain-s3.md`.
